...
import { navigationItems, navigationScreen, type AppScreen } from "./navigation";
...
export function MobileNav({ activeScreen, onMore }: MobileNavProps) {
  ...
  return <nav aria-label="Мобильная навигация" className="shell-mobile-nav">{items.map((item) => <Link className={navigationScreen(activeScreen) === item.id ? "is-active" : ""} href={item.href} key={item.id}><span>{item.icon}</span><small>{item.label}</small></Link>)}<button onClick={onMore} type="button"><span className="mobile-more-icon">•••</span><small>Ещё</small></button></nav>;
}
