...
import { initialTransactionForm, transactionCancelMutation, transactionFormFromRecord, transactionFormWithCounterparty, transactionFormWithPayee, transactionMutation, type TransactionForm } from "./transaction-form";
import { transactionPartyDetail, transactionPartyName } from "./transaction-presenters";

interface TransactionsScreenProps { initialAccountId?: string; onError: (error: unknown) => void; openForm?: boolean; role: WorkspaceRole | null; roleLoading: boolean; }
...
function TransactionDrawer({ accounts, categories, editing, form, isSaving, onChange, onClose, onError, onSave }: {
  ...
  return <div className="transaction-drawer-backdrop" onMouseDown={onClose} role="presentation"><section aria-label={editing ? "Редактирование операции" : "Новая операция"} aria-modal="true" className="transaction-drawer" onMouseDown={(event) => event.stopPropagation()} ref={drawerRef} role="dialog">
    ...
    <form onSubmit={onSave}>
      ...
      <PayeeCombobox initialSelection={editing?.payee ?? null} onChange={(payeeId) => onChange(transactionFormWithPayee(form, payeeId))} onError={onError} value={form.payeeId}/>
      <label><span>Исходный контрагент</span><input maxLength={300} value={form.counterparty} onChange={(event) => onChange(transactionFormWithCounterparty(form, event.target.value))} placeholder="Текст из выписки или источника"/><small>Не изменяется при выборе получателя.</small></label>
...
export function TransactionsScreen({ initialAccountId, onError, openForm = false, role, roleLoading }: TransactionsScreenProps) {
  ...
  async function categorizationApplied(transaction: Transaction) { setTransactions((current) => current.map((item) => item.id === transaction.id ? transaction : item)); setCategorizing(transaction); await load(); }
  ...
  return <section>
    ...
    <div className="panel transaction-ledger">
      ...
      <div className="transaction-table-wrap"><table className="transaction-table"><thead><tr><th><input aria-label="Выбрать страницу" checked={Boolean(transactions.length) && selected.length === transactions.length} onChange={(event) => setSelected(event.target.checked ? transactions.map((item) => item.id) : [])} type="checkbox"/></th><th>Дата</th><th>Получатель / источник</th><th>Категория</th><th>Счёт</th><th>Статус</th><th>Сумма</th><th><span className="sr-only">Действия</span></th></tr></thead><tbody>{transactions.map((transaction) => <tr className={selected.includes(transaction.id) ? "is-selected" : ""} key={transaction.id}><td><input aria-label={`Выбрать ${transactionPartyName(transaction)}`} checked={selected.includes(transaction.id)} onChange={() => toggleSelected(transaction.id)} type="checkbox"/></td><td><time dateTime={transaction.occurred_at}>{new Intl.DateTimeFormat("ru-RU", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(transaction.occurred_at))}</time></td><td><strong>{transactionPartyName(transaction)}</strong><small>{transactionPartyDetail(transaction) ?? transaction.description ?? sourceLabels[transaction.source]}</small></td><td><span className="category-pill">{transaction.splits.length ? `${transaction.splits.length} категории` : transaction.category?.name ?? "Без категории"}</span></td><td><strong>{transaction.account.name}</strong><small>{transaction.transaction_type === "transfer" ? `→ ${transaction.target_account?.name ?? "—"}` : sourceLabels[transaction.source]}</small></td><td><span className={`status-chip status-chip--${transaction.status}`}>{statusLabels[transaction.status]}</span></td><td className={`amount-cell amount-cell--${transaction.transaction_type}`}>{transaction.transaction_type === "expense" ? "− " : transaction.transaction_type === "income" ? "+ " : ""}{formatMoney(transaction.amount, transaction.currency)}</td><td><div className="transaction-row-actions"><button className="text-button" onClick={() => setCategorizing(transaction)} type="button">Правило</button><button className="text-button" onClick={() => startEdit(transaction)} type="button">Изменить</button><button className="text-button" onClick={() => void showHistory(transaction)} type="button">История</button>{transaction.status !== "cancelled" ? <button className="text-button text-button--danger" onClick={() => void cancel(transaction)} type="button">Отменить</button> : null}</div></td></tr>)}</tbody></table>
        <div className="transaction-mobile-list">{transactions.map((transaction) => <article className={selected.includes(transaction.id) ? "transaction-mobile-card is-selected" : "transaction-mobile-card"} key={transaction.id}><div><input aria-label={`Выбрать ${transactionPartyName(transaction)}`} checked={selected.includes(transaction.id)} onChange={() => toggleSelected(transaction.id)} type="checkbox"/><span className={`transaction-type-icon transaction-type-icon--${transaction.transaction_type}`}>{transaction.transaction_type === "income" ? "↓" : transaction.transaction_type === "transfer" ? "↔" : "↑"}</span><div><strong>{transactionPartyName(transaction)}</strong><small>{transactionPartyDetail(transaction) ? `${transactionPartyDetail(transaction)} · ` : ""}{transaction.category?.name ?? sourceLabels[transaction.source]} · {new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "short" }).format(new Date(transaction.occurred_at))}</small></div><b className={`amount-cell--${transaction.transaction_type}`}>{transaction.transaction_type === "expense" ? "− " : transaction.transaction_type === "income" ? "+ " : ""}{formatMoney(transaction.amount, transaction.currency)}</b></div><footer><span className={`status-chip status-chip--${transaction.status}`}>{statusLabels[transaction.status]}</span><button className="text-button" onClick={() => setCategorizing(transaction)} type="button">Правило</button><button className="text-button" onClick={() => startEdit(transaction)} type="button">Изменить</button><button className="text-button" onClick={() => void showHistory(transaction)} type="button">История</button></footer></article>)}</div>
    ...
    {drawerOpen ? <TransactionDrawer accounts={accounts} categories={categories} editing={editing} form={form} isSaving={isSaving} onChange={setForm} onClose={closeDrawer} onError={onError} onSave={(event) => void save(event)}/> : null}
    {history ? <div className="modal-backdrop" role="presentation" onMouseDown={() => setHistory(null)}><section className="history-panel" role="dialog" aria-modal="true" aria-label="История операции" onMouseDown={(event) => event.stopPropagation()}><div className="panel-heading"><div><span className="kicker">Audit log</span><h2>История операции</h2><p>{transactionPartyName(history.transaction)}</p></div><button className="text-button" type="button" onClick={() => setHistory(null)}>Закрыть</button></div><ol className="history-list">{history.items.map((entry) => <li key={entry.id}><span className="history-dot"/><div><strong>{entry.action}</strong><span>{new Intl.DateTimeFormat("ru-RU", { dateStyle: "medium", timeStyle: "medium" }).format(new Date(entry.created_at))}</span>{entry.request_id ? <code>{entry.request_id}</code> : null}</div></li>)}</ol></section></div> : null}
    {categorizing ? <TransactionCategorizationDialog canApply={role === "owner" || role === "editor"} onApplied={categorizationApplied} onClose={() => setCategorizing(null)} onError={onError} roleLoading={roleLoading} transaction={categorizing}/> : null}
  </section>;
}
