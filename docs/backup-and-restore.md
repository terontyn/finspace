...
## Проверка восстановления в чистой среде

Регулярная проверка `local_verified` доказывает, что дамп восстанавливается во временную базу
на том же хосте. Она **не** доказывает, что из резервной копии, внешне сохранённых секретов и
точного релиза можно собрать работающую систему на пустой машине. Это доказывает отдельное
учение: [disaster-recovery-drill.md](disaster-recovery-drill.md).

## Какой релиз поднимать на восстановленной копии

Backup set несёт `alembic_revision`, `finspace_commit` и, если он был, `finspace_tag` — по ним и
выбирается релиз. Если revision копии новее, чем поддерживает выбранное приложение, запускать это
приложение на такой копии нельзя; если старее — сначала restore, затем `alembic upgrade head`
образом целевого релиза. Схема восстановленной базы никогда не понижается молча. Полная таблица и
порядок действий: [upgrade.md](upgrade.md#8-совместимость-restore-и-версии-схемы).
...
Установка на чистый сервер — [production-install.md](production-install.md#9-плановые-проверенные-backup);
ниже тот же порядок вместе с контрактом.
