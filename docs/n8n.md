...
## Опциональность и восстановление

n8n **необязателен**: без него работают приложение, импорт, отчёты, закрытие месяца,
Google-синхронизация и плановый backup. Пропадают только Telegram-бот, напоминания и
автоматические уведомления.

- восстановление финансовых данных от n8n не зависит: источник истины — PostgreSQL;
- версия образа закреплена в Compose, обновление версии — отдельное осознанное решение;
- workflow лежат в репозитории (`n8n/workflows`), а owner-аккаунт, credentials и история
  выполнения — в named volume `finspace_n8n_data`, который в PostgreSQL dump не входит;
- `N8N_ENCRYPTION_KEY` хранится вне Git и вне backup: без него credentials из volume
  расшифровать нельзя, и их придётся создать заново;
- доступ только `127.0.0.1:5678`. Публиковать n8n через tunnel/Funnel нельзя; для
  администрирования с другой машины используйте SSH port forwarding;
- обычный deploy приложения не должен останавливать и пересоздавать n8n, если релиз не
  содержит n8n-specific изменений.
